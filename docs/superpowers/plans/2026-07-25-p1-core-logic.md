...
> **Pivot (v3 recordings, 2026-07-25):** the original plan assumed the rough take rushed **seg-003**. Three real recordings (frozen in commit `5041779`) say otherwise: both takes *honour* seg-003's marked pause, and the genuine rush is **seg-005** (the ARR key stat, ~+25% over baseline). The red tick therefore lives on **seg-005**, not seg-003. seg-003 remains only "the segment with the marked pause". seg-006 is faster still in raw WPM but is a 2–3s end-blurt — too short a span to be a reliable pace verdict, so Task 9 adds a minimum-duration guard. The whole point is honest thresholds over real delivery, not a red tick hand-placed on a chosen segment.
...
Among the two key-point segments (seg-003, seg-005), `seg-005` must be the faster and must clear seg-003 by a clear margin (measured: 180 vs 150 WPM, 1.2×). seg-006 is faster still but is a 2.3s end-blurt, not a key point — Task 9's duration guard keeps it out of pace verdicts. If seg-005 does not outpace seg-003, the rough take did not rush the key stat enough; re-record rather than lowering the threshold — the demo depends on this being real.
...
This task ships the demo. After it, `seg-005` (the rushed ARR key stat) carries one high-severity `stress_mismatch` and Ops Canvas has a trace to render. Filler, pause and pacing rules land in Task 9.
...
Tuning direction (v3): the red tick must fall on **seg-005**, and seg-003 must stay clean. seg-006's 313 WPM end-blurt inflates `paceStdDev` to ~71.8, so at the shipped `paceDriftSigma` of 1.5 the rush ceiling (~252 WPM) is unreachable and *nothing* fires. **Lower `paceDriftSigma` toward ~0.4** in `thresholds.ts` until seg-005 (180 WPM) clears the ceiling while seg-003 (150 WPM) does not: at 0.4 the ceiling is ~172.6 WPM (measured baseline 143.9). If seg-003 *also* fires, the band is too tight — nudge sigma back up. Do not edit the test. (The low sigma is compensating for one short-span outlier in the baseline's spread; Task 9's duration guard keeps that same outlier out of the pacing verdicts.)
...
**Files:**
- Modify: `packages/core-logic/src/thresholds.ts` (add `minPaceVerdictSec`)
...
4. `pacing` skips any segment whose rounded span is under `minPaceVerdictSec` (4s) — a ~2-3s window gives one breath, not a pace. This is what keeps seg-006's 313/218 WPM end-blurt from firing pacing in either take.
...
describe('correlateSegments — filler, pause, pacing', () => {
  ...
  it('pacing is two-sided but only on ordinary, long-enough, non-stress segments', () => {
    ...
    // stress issue. With paceDriftSigma tuned to ~0.4 the rough take flags
    // seg-001 and seg-002 (slow) and seg-004 (fast); the clean take flags seg-001
    // and seg-002. (CorrelationResult carries no alignments, so re-derive spans.)
    const spans = new Map(
...
- [ ] **Step 3: Add the pace-verdict duration guard to `thresholds.ts`, then the filler-run helper**

First **edit** `packages/core-logic/src/thresholds.ts` (Task 1 already created it, so this
is an edit, not a create) to add one field to `THRESHOLDS`, after `paceDriftSigma`:
...
```ts
   ...
   * breath. The pacing rule skips such segments. seg-006's end-blurt (313 WPM
   * over 2.3s rough / 218 WPM over 3.3s clean) must not fire pacing in either take.
   */
  minPaceVerdictSec: 4,
```

Then add the filler-run helper:
...
export function correlateSegments(
  ...
  for (const segment of segments) {
    ...
    // a slow open (seg-001, ~-34%) is as coachable as a fast line.
...
```bash
git add packages/core-logic/src/thresholds.ts packages/core-logic/src/correlate.ts packages/core-logic/src/correlate.test.ts
...
Expected: `rough` reports exactly 1 high on `seg-005` (stress_mismatch); `clean` reports 0 high (all issues `low`); all contract tests pass. See the expected-report sketches below for the full v3 issue shape.

> **Task 3 is already done.** `scripts/transcribe.mjs` exists and the v3 transcript
> fixtures (`transcript.rough.json`, `transcript.clean.json`) were regenerated in
> commit `5041779` ("regenerate transcript fixtures from v3 takes"). Task 12 only
> regenerates the *report* fixtures from those transcripts — do not re-run Deepgram.

**Expected v3 report shape** (measured against the committed transcripts via the real
pipeline, `paceDriftSigma` ~0.4, `minPaceVerdictSec` 4; every value below is real, not
a guess). All emitted floats are 1-decimal; ids are assigned after the timestamp sort.

`report.rough.json` — 91 transcript words, `durationSec` 38.7, `avgPaceWpm` 143.9,
`fillerCount` 2, `nextStep.kind` `calendar_reminder` (Northwind investor call), 6 issues:

| id | severity | type | segment | ~timestamp |
|---|---|---|---|---|
| iss-001 | low | pacing | seg-001 | 0.5 |
| iss-002 | low | filler | seg-001 | 3.4 |
| iss-003 | low | pacing | seg-002 | 8.4 |
| iss-004 | low | filler | seg-002 | 14.0 |
| iss-005 | low | pacing | seg-004 | 25.6 |
| iss-006 | **high** | **stress_mismatch** | **seg-005** | 30.6 |

`report.clean.json` — 86 transcript words, `durationSec` 40.9, `avgPaceWpm` 146.6,
`fillerCount` 0, `nextStep.kind` `draft_note` (mentor Priya), all-`low`, 2 issues:

| id | severity | type | segment | ~timestamp |
|---|---|---|---|---|
| iss-001 | low | pacing | seg-001 | 1.1 |
| iss-002 | low | pacing | seg-002 | 7.6 |

Notes on the v3 numbers: (1) the red tick is **seg-005**, the ARR stat (180 vs 143.9
WPM baseline, ~+25%), not seg-003. (2) **No pause issue** in either take — both honour
seg-003's marked pause. (3) rough `fillerCount` is 2: the tagged fillers are one "like"
(seg-001) and the contiguous "like i mean" run (seg-002); clean's spoken "like"s all
...
| Failure | Knob |
|---|---|
| No high-severity issue (v3 default: at `paceDriftSigma` 1.5 the ceiling is ~252 WPM and nothing fires) | **lower** `paceDriftSigma` toward ~0.4 until seg-005 (180 WPM) clears the ceiling. seg-006's short-span 313 WPM inflates `paceStdDev` to ~71.8, which is why the sigma is far below 1.0. If still none, the take did not rush the key stat — re-record |
| More than one high-severity issue, or seg-003 also fires | the band is too tight — **raise** `paceDriftSigma` slightly (e.g. 0.4 → 0.5). seg-003 (150 WPM) must stay below the ceiling; seg-005 (180 WPM) must stay above it |
| Clean take has a non-`low` issue | `fixtures.test.ts` (~line 77) requires **every** clean-take issue to be `low` — stricter than "no high". Raise the `filler.density` / in-key-point thresholds so an isolated hedge stays `low`; if a lone hard filler lands in a key point, make `filler.in-key-point`'s verdict depend on run length (single filler → `low`) by adjusting the `SEVERITY_RULES` table. Never edit the fixture test. (v3 clean has no fillers, so this is unlikely to trigger.) |
| `fillerCount` disagrees with filler issues | a filler run spans a segment boundary; widen the run-grouping window |
| A pause issue appears on `seg-003` | it should not — both v3 takes honour the pause. If one fires, `markedPauseHonouredRatio` is too high; **lower** it (0.5 is correct for the v3 numbers: precedingPause ~0.8s vs median ~0.5s, threshold 0.25s) |
| seg-006 fires a pacing issue | `minPaceVerdictSec` is too low; it must be > seg-006's span (2.3s rough / 3.3s clean). 4 is correct |
| Too many pacing issues | raise `paceDriftSigma`, but note seg-005 must stay flagged — there is a floor below which raising sigma drops the red tick |
